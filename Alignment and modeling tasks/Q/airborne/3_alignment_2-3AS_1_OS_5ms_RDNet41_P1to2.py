...
seted_parameter = {'setted_SFT_path':['SFT_log_0_time_shift_0.001_0','SFT_log_0_time_shift_0.001_5','SFT_log_0_time_shift_0.001_10','SFT_log_0_time_shift_0.001_15','SFT_log_0_time_shift_0.001_20'],
                   'setted_img_transform_height':56,'setted_img_transform_width':56,
                   'setted_batch_size':512,'seted_num_epochs':100, 'seted_lr':0.0002,
                   'setted_loss_and_accuracy_file_name':'Loss_and_accuracy_AS_1_and_OS_5ms_RDNet41_P1to2','seted_sample_number/2':4500,
...
class DenseNet(nn.Module):
    ...
    def __init__(self, layers: object, k, theta, num_classes) -> object:
        ...
        self.conv = BN_Conv2d(3, 2*k, 1, 1, 1)
...
for cyclic_number in cyclic_array:
	...
	for file_number,png_list_5ms in enumerate(png_list): #处理时移数据和采样数据建立数据集
		...
		selected_label_1_index = random.sample(label_1_index, 1500)
		#selected_label_0_index = random.sample(label_0_index, number_label_1)
		selected_label_0_index = random.sample(label_0_index, 3000)
		...
		finnal_label = torch.cat((torch.zeros(3000),torch.ones(1500)),dim=0)
		...
		train_idx, val_idx = train_test_split(
                                              ...
                                              test_size=0.3,  # 验证集比例
                                              shuffle=True,
                                              stratify=labels,  # 关键的分层参数
                                              random_state=1    # 随机种子
                                              )
		train_dataset = torch.utils.data.Subset(self_dataset, train_idx)
		valid_dataset = torch.utils.data.Subset(self_dataset, val_idx)
		batch_size = seted_parameter['setted_batch_size']

		train_d1_for_CNN = DataLoader(train_dataset, batch_size, shuffle=True)# 注意：验证集通常不shuffle
		valid_d1_for_CNN = DataLoader(valid_dataset, batch_size, shuffle=False)  # 注意修正你的中文括号

		total_samples = 0
		positive_samples = 0  # 假设标签1表示正样本
		# 遍历训练集 DataLoader
		for _, labels in valid_d1_for_CNN:
			batch_size = labels.size(0)
			total_samples += batch_size
			#print(labels)
			positive_samples += labels.sum().item()  # 使用.item()获取Python数值
		print(f"训练集总样本数: {total_samples}")
		print(f"训练集正样本数: {positive_samples}")
		print(f"训练集负样本数: {total_samples - positive_samples}")
		print(f"正样本比例: {positive_samples/total_samples:.2%}")
