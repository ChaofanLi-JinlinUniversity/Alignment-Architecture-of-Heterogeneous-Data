...
seted_parameter = {'setted_SFT_path':['SFT_log_0_time_shift_0.001_0','SFT_log_0_time_shift_0.001_5','SFT_log_0_time_shift_0.001_10','SFT_log_0_time_shift_0.001_15','SFT_log_0_time_shift_0.001_20'],
                   'setted_img_transform_height':112,'setted_img_transform_width':112,
                   'setted_batch_size':512,'seted_num_epochs':100, 'seted_lr':0.0002,
                   'setted_loss_and_accuracy_file_name':'Loss_and_accuracy_AS_1_and_OS_5ms_RMNet','seted_sample_number/2':4500,
...
class MiniCNN4(nn.Module):
    def __init__(self):
        ...
        self.feature = nn.Sequential(nn.Conv2d(3, 2, kernel_size=3, stride=2, padding=1,bias=False),
                                    nn.BatchNorm2d(2),
                                    nn.ReLU(),
                                    nn.AvgPool2d(32),
                                    nn.Flatten()
...
for cyclic_number in cyclic_array:
	...
	for file_number,png_list_5ms in enumerate(png_list): #处理时移数据和采样数据建立数据集
		...
		model.features[0][0] = nn.Conv2d(3, 16, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
