...
seted_parameter = {'setted_tif_path_1':'2-2/dynamic_threshold_adaptivecutted_tif','setted_tif_path_2':'2-3/dynamic_threshold_adaptivecutted_tif',
                   'setted_background_path':'1_background',
                   ...
                   'setted_batch_size':256,'seted_num_epochs':100, 'seted_lr':0.00001,
                   'model_and_loss_accuracy_path':'Model_Loss_and_Accuracy_D_no_normalization'
                  }
label_path_1 =  r'2-2label&OS.xls'
label_path_2 =  r'2-3label&OS.xls'
...
def caculate_and_sample_file_list(label_file,tif_list): #选择等比例的正负样本
...
self_dataset = Self_Dataset1(tif_flielist, tif_label)
#mean, std = calculate_mean_std(self_dataset)
#print(f'mean:',mean)
#print(f'std',std)
##################################################################################################################################################
##################################################################################################################################################
#for image,label in self_dataset:
#	print(image)
#	print(label.shape)
...
squeezenet1.features[0] = nn.Conv2d(1,64,kernel_size = (3,3),stride = (2,2))
#print(squeezenet1)
...
#num_ftrs = resnet18.fc.in_features                                                    # 获取原fc层的输入特征数
#resnet18.fc = torch.nn.Linear(num_ftrs, 2)                                            # 替换fc层
#a = torch.ones(1,3,224,224)
#print(squeezenet1(a).shape)
#print(densenet)

input = torch.ones(1, 1, 224, 224)
output = squeezenet1(input)
print(output.shape)
...
#Training Definition
def train(model, num_epochs, train_d1, valid_d1,path):
	...
	for epoch in range(num_epochs):
		...
		torch.save(model, os.path.join(path,'model_squeezenetD1_non{}.pt'.format(epoch+1)))
...
#torch.manual_seed(1)
start_time = time.time()
hist = train(squeezenet1, seted_parameter['seted_num_epochs'], train_d1_for_CNN, valid_d1_for_CNN,seted_parameter['model_and_loss_accuracy_path'])
#end_time = time.time()
...
df_hist.to_excel(os.path.join(seted_parameter['model_and_loss_accuracy_path'],'Hist_loss_and_accuracyD.xlsx'), index=False, header=['Epoch','Train_loss','Valid_loss','Train_acc','Valid_acc','TP','TN','FP','FN'])