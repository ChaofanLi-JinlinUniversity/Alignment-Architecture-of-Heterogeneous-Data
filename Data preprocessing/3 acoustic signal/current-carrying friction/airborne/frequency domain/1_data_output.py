...
setted_parameter = {
                     'cycle_start_number':int(20),'cycle_interval':int(5),'cycle_end_number':int(21),'time_shift_length':0.001,
...
def signal_cutting_and_output_FFT_cycle( cycle_start_number,cycle_interval,cycle_end_number,time_shift_length,
    ...
    saved_file_list = [f"FFT_log_{log_process}_time_shift_{time_shift_length}_{i}" for i in range(cycle_start_number, cycle_end_number+1,cycle_interval)]
    ...
    for saved_file in saved_file_list:
        ...
        for file_0 in sorted(csv_file_0, key = sort_by_number_in_filename):   #把路径下的文件按照时间顺序排序并依次读取 key = os.path.getctime
            ...
            for i in range(int(cycle_number)):
                ...
                sum_amps_0 = np.sum(amps_0)
                amps_0 = 1000*amps_0/sum_amps_0
                #print(np.sum(amps_0))
                df = pd.DataFrame({'fre': fs_0,
                                   'amp': amps_0})
                df.to_excel(os.path.join(saved_file,'fft{}.xlsx'.format(number)), index=False)
                number = number+1
               


signal_cutting_and_output_FFT_cycle(**setted_parameter)
print('finsh')#ABC