...
setted_parameter = {
                     'cycle_start_number':int(0),'cycle_interval':int(5),'cycle_end_number':int(2),'time_shift_length':0.001,
                     'sorted_number_location':int(4),'origin_data_path':'wu-2/Source_data_1',
                     'cutted_signal_time':0.4, 'sampling_frequency':int(2000000),'remove_offset':int(1),'log_process':int(0),'guiyihua':int(1),
                     'seted_seg_length':9650, 'selected_highest_requency':34000
                   }
...
def signal_cutting_and_output_SFT_cycle( cycle_start_number,cycle_interval,cycle_end_number,time_shift_length,
    ...
    saved_file_list = [f"wu-2/SFT1_log_{log_process}_time_shift_{time_shift_length}_{i}" for i in range(cycle_start_number, cycle_end_number+1,cycle_interval)]
