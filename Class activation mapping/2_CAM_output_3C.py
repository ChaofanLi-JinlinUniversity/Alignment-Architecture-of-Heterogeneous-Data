			...
			if is_correct[index].cpu().item() == 1:
				...
				with GradCAM(model=model, target_layers=target_layers) as cam:
					...
					for grayscale_cam, tensor in zip(grayscale_cams, input_tensor):
						...
						myimshows([rgb_img, grayscale_cam, visualization], ["image", "cam", "image + cam"],fname=os.path.join(seted_parameter['setted_CAM_output'], "cam_{}_{}.jpg".format(pred_result[index].cpu().item(),index)))
